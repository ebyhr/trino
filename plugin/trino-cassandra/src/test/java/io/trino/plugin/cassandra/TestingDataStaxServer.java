...
public class TestingDataStaxServer
        implements Closeable
{
    private static final Logger log = Logger.get(TestingDataStaxServer.class);

    private static final String LATEST_TAG = "6.8.18";
    private static final String DEFAULT_TAG = "5.1.5";
    ...
    public TestingDataStaxServer()
    {
        this(LATEST_TAG);
    }

    public TestingDataStaxServer(String version)
    {
        container = new GenericContainer<>("datastax/dse-server:" + version)
                .withEnv("DS_LICENSE", "accept")
                .withExposedPorts(PORT);
        container.start();

        Cluster.Builder clusterBuilder = Cluster.builder()
                .addContactPointsWithPorts(new InetSocketAddress(container.getContainerIpAddress(), container.getMappedPort(PORT)))
                .withMaxSchemaAgreementWaitSeconds(60);

        session = new CassandraSession(JsonCodec.listJsonCodec(ExtraColumnMetadata.class), clusterBuilder.build(), new Duration(1, MINUTES));
    }
